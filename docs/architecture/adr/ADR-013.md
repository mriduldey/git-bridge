...
RepoFerry is designed as a long-lived open-source ecosystem consisting of multiple packages that evolve together.
...
RepoFerry adopts a **lockstep-versioned pnpm monorepo** with task-oriented build orchestration.
...
RepoFerry is organized as a **multi-package monorepo** managed with **pnpm workspaces**.
...
RepoFerry publishes **dual ESM/CJS packages**.
...
RepoFerry follows **Semantic Versioning (SemVer)** for all public packages.
...
```text
repoferry                    1.2.0
@repoferry/core              1.2.0
@repoferry/provider-github   1.2.0
@repoferry/testing           1.2.0
```
...
RepoFerry supports progressive release channels.
...
RepoFerry automates release engineering wherever practical.
...
RepoFerry classifies dependencies as:
...
RepoFerry favors predictable long-term evolution.
...
This ADR defines the build, packaging, and release engineering architecture of RepoFerry.
...
ADR-013 establishes the build and release engineering architecture of RepoFerry.
