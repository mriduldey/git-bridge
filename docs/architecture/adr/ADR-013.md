...
SourceAxis is designed as a long-lived open-source ecosystem consisting of multiple packages that evolve together.
...
SourceAxis adopts a **lockstep-versioned pnpm monorepo** with task-oriented build orchestration.
...
SourceAxis is organized as a **multi-package monorepo** managed with **pnpm workspaces**.
...
SourceAxis publishes **dual ESM/CJS packages**.
...
SourceAxis follows **Semantic Versioning (SemVer)** for all public packages.
...
```text
sourceaxis                    1.2.0
@sourceaxis/core              1.2.0
@sourceaxis/provider-github   1.2.0
@sourceaxis/testing           1.2.0
```
...
SourceAxis supports progressive release channels.
...
SourceAxis automates release engineering wherever practical.
...
SourceAxis classifies dependencies as:
...
SourceAxis favors predictable long-term evolution.
...
This ADR defines the build, packaging, and release engineering architecture of SourceAxis.
...
ADR-013 establishes the build and release engineering architecture of SourceAxis.
