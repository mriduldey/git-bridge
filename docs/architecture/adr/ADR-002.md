...
GitBridge aims to provide a provider-neutral API for interacting with Git repositories while remaining independent of any hosting platform.

The public API is the library's most valuable long-term asset.

Unlike internal implementation details, public contracts become part of GitBridge's semantic versioning guarantees and must therefore prioritize:
...
GitBridge adopts a **service-oriented public API** centered around a small number of high-level service objects and immutable value models.
...
The application interacts only with GitBridge.
...
GitBridge distinguishes between objects that perform work and objects that represent data.
...
Rather than requiring every operation to specify a branch or commit, GitBridge introduces the concept of a Repository Reference.
...
GitBridge models Git concepts rather than provider concepts.

```mermaid
classDiagram

GitBridgeClient --> Repository
...
GitBridge exposes a single primary entry point.
...
GitBridge intentionally avoids global mutable configuration.
...
Every GitBridgeClient owns an independent configuration graph.
...
GitBridge organizes repository operations into cohesive capability services rather than exposing a large collection of unrelated methods.
...
GitBridge distinguishes between content formats instead of returning a single ambiguous type.
...
GitBridge exposes explicit operations.
...
GitBridge supports both simple and advanced search.
...
GitBridge exposes a dedicated convenience operation.
...
GitBridge therefore distinguishes:
...
Authentication belongs exclusively to GitBridgeClient configuration.
...
GitBridge's public API remains provider-neutral.
...
GitBridge does not expose provider pagination models.
...
GitBridge adopts selective fluency.
...
Public types are part of GitBridge's compatibility contract and therefore receive the same level of stability as runtime APIs.
...
GitBridge intentionally avoids exposing advanced TypeScript constructs unless they significantly improve developer experience.
...
GitBridge adopts an **exception-based programming model**.
...
This ADR defines the public programming model for GitBridge.
...
This ADR establishes the complete public programming model for GitBridge.
