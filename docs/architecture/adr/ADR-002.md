...
SourceAxis aims to provide a provider-neutral API for interacting with Git repositories while remaining independent of any hosting platform.

The public API is the library's most valuable long-term asset.

Unlike internal implementation details, public contracts become part of SourceAxis's semantic versioning guarantees and must therefore prioritize:
...
SourceAxis adopts a **service-oriented public API** centered around a small number of high-level service objects and immutable value models.
...
The application interacts only with SourceAxis.
...
SourceAxis distinguishes between objects that perform work and objects that represent data.
...
Rather than requiring every operation to specify a branch or commit, SourceAxis introduces the concept of a Repository Reference.
...
SourceAxis models Git concepts rather than provider concepts.

```mermaid
classDiagram

SourceAxisClient --> Repository
...
SourceAxis exposes a single primary entry point.

```ts
import { SourceAxisClient } from "sourceaxis";
```
...
SourceAxis intentionally avoids global mutable configuration.
...
Every SourceAxisClient owns an independent configuration graph.
...
SourceAxis organizes repository operations into cohesive capability services rather than exposing a large collection of unrelated methods.
...
SourceAxis distinguishes between content formats instead of returning a single ambiguous type.
...
SourceAxis exposes explicit operations.
...
SourceAxis supports both simple and advanced search.
...
SourceAxis exposes a dedicated convenience operation.
...
SourceAxis therefore distinguishes:
...
Authentication belongs exclusively to SourceAxisClient configuration.
...
SourceAxis's public API remains provider-neutral.
...
SourceAxis does not expose provider pagination models.
...
SourceAxis adopts selective fluency.
...
Public types are part of SourceAxis's compatibility contract and therefore receive the same level of stability as runtime APIs.
...
SourceAxis intentionally avoids exposing advanced TypeScript constructs unless they significantly improve developer experience.
...
SourceAxis adopts an **exception-based programming model**.
...
This ADR defines the public programming model for SourceAxis.
...
This ADR establishes the complete public programming model for SourceAxis.
