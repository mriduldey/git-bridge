...
SourceAxis requires a provider-neutral caching architecture that improves performance while preserving correctness and predictable behavior.

---

# 2. Decision

SourceAxis adopts a **client-scoped, registry-driven caching architecture**.
...
SourceAxis avoids premature optimization.
...
Rather than embedding knowledge of individual cache types inside a CacheManager, SourceAxis uses a **CacheRegistry**.
...
SourceAxis defines logical cache layers.
...
SourceAxis follows six caching principles.
...
SourceAxis supports multiple cache behaviors.
...
SourceAxis favors predictable memory usage.
...
SourceAxis includes several architectural optimizations.
...
This ADR defines the caching architecture of SourceAxis.
...
ADR-009 establishes the caching, performance, and resource management architecture of SourceAxis.
