...
RepoFerry requires a provider-neutral caching architecture that improves performance while preserving correctness and predictable behavior.

---

# 2. Decision

RepoFerry adopts a **client-scoped, registry-driven caching architecture**.
...
RepoFerry avoids premature optimization.
...
Rather than embedding knowledge of individual cache types inside a CacheManager, RepoFerry uses a **CacheRegistry**.
...
RepoFerry defines logical cache layers.
...
RepoFerry follows six caching principles.
...
RepoFerry supports multiple cache behaviors.
...
RepoFerry favors predictable memory usage.
...
RepoFerry includes several architectural optimizations.
...
This ADR defines the caching architecture of RepoFerry.
...
ADR-009 establishes the caching, performance, and resource management architecture of RepoFerry.
