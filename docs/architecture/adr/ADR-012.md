...
For RepoFerry, testing verifies that:
...
Testing therefore forms part of RepoFerry's architectural governance.

---

# 2. Decision

RepoFerry adopts a **multi-layer verification architecture**.
...
RepoFerry emphasizes fast deterministic tests.
...
RepoFerry provides a reusable Contract Test Kit.
...
End-to-End (E2E) tests validate the complete RepoFerry runtime against real provider environments.
...
TypeScript types are part of RepoFerry's public API.
...
RepoFerry maintains multiple reference repositories representing different usage patterns.
...
RepoFerry treats mutation testing as:
...
RepoFerry maintains representative consumer applications.
...
RepoFerry follows a layered CI strategy.
...
RepoFerry prioritizes architectural verification instead.
...
This ADR defines the testing architecture of RepoFerry.
...
ADR-012 establishes the testing, verification, and quality architecture of RepoFerry.
...
> **Testing is an architectural capability that continuously verifies RepoFerry's contracts, boundaries, compatibility, and long-term maintainability. Confidence comes from layered verification rather than coverage alone, ensuring every release preserves architectural integrity and developer trust.**