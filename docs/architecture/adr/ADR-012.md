...
For SourceAxis, testing verifies that:
...
Testing therefore forms part of SourceAxis's architectural governance.

---

# 2. Decision

SourceAxis adopts a **multi-layer verification architecture**.
...
SourceAxis emphasizes fast deterministic tests.
...
SourceAxis provides a reusable Contract Test Kit.
...
End-to-End (E2E) tests validate the complete SourceAxis runtime against real provider environments.
...
TypeScript types are part of SourceAxis's public API.
...
SourceAxis maintains multiple reference repositories representing different usage patterns.
...
SourceAxis treats mutation testing as:
...
SourceAxis maintains representative consumer applications.
...
SourceAxis follows a layered CI strategy.
...
SourceAxis prioritizes architectural verification instead.
...
This ADR defines the testing architecture of SourceAxis.
...
ADR-012 establishes the testing, verification, and quality architecture of SourceAxis.
...
> **Testing is an architectural capability that continuously verifies SourceAxis's contracts, boundaries, compatibility, and long-term maintainability. Confidence comes from layered verification rather than coverage alone, ensuring every release preserves architectural integrity and developer trust.**