...
GitBridge addresses this problem by providing a single, provider-neutral abstraction layer for interacting with Git repositories.

Applications interact only with GitBridge, regardless of where a repository is hosted.

---

# 2. Decision

GitBridge will be designed as a provider-agnostic TypeScript library.

Applications communicate exclusively through GitBridge's public API.
...
GitBridge should allow developers to work with repositories through one intuitive API without needing to understand provider-specific SDKs or REST APIs.
...
Applications communicate only with GitBridge.
...
GitBridge follows these foundational principles.
...
GitBridge evolves incrementally.
...
Applications would become tightly coupled to provider implementations, defeating the purpose of GitBridge.
...
This ADR establishes the architectural foundation for GitBridge.
