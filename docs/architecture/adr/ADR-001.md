...
SourceAxis addresses this problem by providing a single, provider-neutral abstraction layer for interacting with Git repositories.

Applications interact only with SourceAxis, regardless of where a repository is hosted.

---

# 2. Decision

SourceAxis will be designed as a provider-agnostic TypeScript library.

Applications communicate exclusively through SourceAxis's public API.
...
SourceAxis should allow developers to work with repositories through one intuitive API without needing to understand provider-specific SDKs or REST APIs.
...
Applications communicate only with SourceAxis.
...
SourceAxis follows these foundational principles.
...
SourceAxis evolves incrementally.
...
Applications would become tightly coupled to provider implementations, defeating the purpose of SourceAxis.
...
This ADR establishes the architectural foundation for SourceAxis.
