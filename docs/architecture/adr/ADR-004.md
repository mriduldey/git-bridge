...
The Core package is the execution engine of GitBridge.
...
GitBridge adopts a **Ports & Adapters runtime** implemented using a **layered architecture**.
...
GitBridge adopts a **hybrid Layered + Hexagonal Architecture**.
...
The Domain layer models GitBridge concepts.
...
Applications interact only through GitBridgeClient.
...
Application->>GitBridgeClient: new GitBridgeClient(config)

GitBridgeClient->>ConfigurationResolver: Resolve defaults

ConfigurationResolver-->>GitBridgeClient: Effective configuration

GitBridgeClient->>ProviderRegistry: Register providers

ProviderRegistry-->>GitBridgeClient: Registry ready

GitBridgeClient-->>Application: Client instance
```
...
Application->>GitBridgeClient: open(url)

GitBridgeClient->>RepositoryFactory: create(url)
...
RepositoryFactory-->>GitBridgeClient: Repository

GitBridgeClient-->>Application: Repository
```
...
This ownership rule is fundamental to GitBridge.
...
GitBridge intentionally avoids heavyweight IoC containers.
...
GitBridge intentionally avoids:
...
| Public API | Expose stable GitBridge errors |
...
This ADR defines the internal runtime architecture of the `@gitbridge/core` package.
...
ADR-004 defines the internal runtime architecture of GitBridge Core.
...
Together with ADR-001 through ADR-003, this document defines the execution model that every implementation of GitBridge Core must follow.
