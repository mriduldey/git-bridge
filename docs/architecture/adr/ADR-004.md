...
The Core package is the execution engine of RepoFerry.
...
RepoFerry adopts a **Ports & Adapters runtime** implemented using a **layered architecture**.
...
RepoFerry adopts a **hybrid Layered + Hexagonal Architecture**.
...
The Domain layer models RepoFerry concepts.
...
Applications interact only through RepoFerryClient.
...
Application->>RepoFerryClient: new RepoFerryClient(config)

RepoFerryClient->>ConfigurationResolver: Resolve defaults

ConfigurationResolver-->>RepoFerryClient: Effective configuration

RepoFerryClient->>ProviderRegistry: Register providers

ProviderRegistry-->>RepoFerryClient: Registry ready

RepoFerryClient-->>Application: Client instance
```
...
Application->>RepoFerryClient: open(url)

RepoFerryClient->>RepositoryFactory: create(url)
...
RepositoryFactory-->>RepoFerryClient: Repository

RepoFerryClient-->>Application: Repository
```
...
This ownership rule is fundamental to RepoFerry.
...
RepoFerry intentionally avoids heavyweight IoC containers.
...
RepoFerry intentionally avoids:
...
| Public API | Expose stable RepoFerry errors |
...
This ADR defines the internal runtime architecture of the `@repoferry/core` package.
...
ADR-004 defines the internal runtime architecture of RepoFerry Core.
...
Together with ADR-001 through ADR-003, this document defines the execution model that every implementation of RepoFerry Core must follow.
