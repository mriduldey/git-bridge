...
SourceAxis supports multiple Git hosting providers.
...
SourceAxis adopts a **Strategy-based Authentication Architecture**.
...
SourceAxis never expands permissions automatically.
...
SourceAxis adopts a Strategy Pattern.
...
SourceAxis supports multiple independent clients.

Example:

```ts
const publicClient = new SourceAxisClient(...);

const enterpriseClient = new SourceAxisClient(...);
```
...
SourceAxis supports multiple authentication mechanisms through the same
Authentication Strategy abstraction.

Authentication type is an implementation detail.

Applications interact only with SourceAxis contracts.
...
SourceAxis treats API keys as another credential type.
...
SourceAxis delegates refresh responsibility to Authentication Strategy.
...
SourceAxis accepts credentials.
...
Authentication state belongs to a single SourceAxisClient.
...
Authentication failures follow the standard SourceAxis error pipeline.
...
Application->>SourceAxisClient: Create

SourceAxisClient->>Authentication Strategy: Configure

Authentication Strategy-->>SourceAxisClient: Ready
```
...
This ADR defines the authentication architecture of SourceAxis.
...
ADR-006 establishes the authentication architecture of SourceAxis.
...
Together with ADR-004 and ADR-005, this document completes SourceAxis's runtime identity architecture and provides the foundation for the Transport, Error, Caching, and Observability subsystems defined in subsequent ADRs.