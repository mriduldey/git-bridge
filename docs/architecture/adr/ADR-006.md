...
RepoFerry supports multiple Git hosting providers.
...
RepoFerry adopts a **Strategy-based Authentication Architecture**.
...
RepoFerry never expands permissions automatically.
...
RepoFerry adopts a Strategy Pattern.
...
RepoFerry supports multiple independent clients.
...
RepoFerry supports multiple authentication mechanisms through the same
Authentication Strategy abstraction.

Authentication type is an implementation detail.

Applications interact only with RepoFerry contracts.
...
RepoFerry treats API keys as another credential type.
...
RepoFerry delegates refresh responsibility to Authentication Strategy.
...
Authentication state belongs to a single RepoFerryClient.
...
Authentication failures follow the standard RepoFerry error pipeline.
...
Application->>RepoFerryClient: Create

RepoFerryClient->>Authentication Strategy: Configure

Authentication Strategy-->>RepoFerryClient: Ready
```
...
This ADR defines the authentication architecture of RepoFerry.
...
ADR-006 establishes the authentication architecture of RepoFerry.
...
Together with ADR-004 and ADR-005, this document completes RepoFerry's runtime identity architecture and provides the foundation for the Transport, Error, Caching, and Observability subsystems defined in subsequent ADRs.