...
SourceAxis is intended for use in production applications, automation systems, developer tools, and long-running services.

Consumers require visibility into runtime behavior without coupling SourceAxis to any particular logging, tracing, or monitoring framework.
...
SourceAxis adopts a **provider-neutral observability architecture** built around a `DiagnosticsService`.
...
> "What happened inside SourceAxis?"
...
SourceAxis follows these principles:
...
This allows diagnostic consumers to evolve independently from SourceAxis releases.
...
SourceAxis adopts an **Observer / Publisher–Subscriber** model.
...
SourceAxis never logs directly.
...
SourceAxis depends only on abstractions.
...
SourceAxis supports distributed tracing through provider-neutral abstractions.
...
SourceAxis does not implement a tracing backend.
...
SourceAxis does not depend on OpenTelemetry packages.

Instead, adapters may translate SourceAxis trace contracts into:
...
SourceAxis exposes diagnostics through explicit services.
...
SourceAxis never exposes EventEmitter directly.
...
Subscribers remain external to SourceAxis.
...
Would couple SourceAxis to logging frameworks.
...
This ADR defines the observability architecture of SourceAxis.
...
ADR-010 establishes the observability architecture of SourceAxis.
...
> **Observability is a passive, provider-neutral capability that exposes runtime behavior through stable contracts without influencing execution, allowing SourceAxis to integrate with any logging, metrics, or tracing ecosystem while remaining framework-independent.**