...
SourceAxis is designed as a long-lived open-source ecosystem rather than a single npm package.
...
SourceAxis adopts a **multi-package monorepo** architecture.
...
| Capability | Owner |
|------------|-------|
| Core runtime | @sourceaxis/core |
| GitHub integration | @sourceaxis/provider-github |
| Shared testing | @sourceaxis/testing |
| Common utilities | @sourceaxis/shared |
...
SourceAxis adopts a **multi-package monorepo** managed through a shared workspace.

```mermaid
flowchart TD

Repo["SourceAxis Repository"]
...
SourceAxis distinguishes between **public**, **internal**, and **future** packages.
...
```text
@sourceaxis/internal-build

@sourceaxis/internal-config

@sourceaxis/internal-devtools
```
...
```text
@sourceaxis/provider-gitlab

@sourceaxis/provider-bitbucket

@sourceaxis/provider-azure

@sourceaxis/provider-gitea

@sourceaxis/provider-local

@sourceaxis/provider-zip

@sourceaxis/cache-memory

@sourceaxis/cache-redis
```
...
# 9. Core Package (@sourceaxis/core)

The Core package is the architectural heart of SourceAxis.
...
Each provider package adapts one hosting platform to SourceAxis's provider contracts.
...
Providers translate external systems into SourceAxis concepts.
...
# 11. Shared Package (@sourceaxis/shared)
...
# 12. Testing Package (@sourceaxis/testing)
...
SourceAxis is designed for ecosystem growth.
...
```ts
import { Provider } from "@sourceaxis/core";
```
...
SourceAxis targets modern JavaScript ecosystems.
...
This ADR defines the package-level architecture of SourceAxis.
...
This ADR establishes the package architecture that supports the long-term evolution of SourceAxis.
...
Together with ADR-001 and ADR-002, this document provides the structural foundation upon which the remainder of SourceAxis is implemented.
