...
RepoFerry is designed as a long-lived open-source ecosystem rather than a single npm package.
...
RepoFerry adopts a **multi-package monorepo** architecture.
...
| Capability | Owner |
|------------|-------|
| Core runtime | @repoferry/core |
| GitHub integration | @repoferry/provider-github |
| Shared testing | @repoferry/testing |
| Common utilities | @repoferry/shared |
...
RepoFerry adopts a **multi-package monorepo** managed through a shared workspace.

```mermaid
flowchart TD

Repo["RepoFerry Repository"]
...
RepoFerry distinguishes between **public**, **internal**, and **future** packages.
...
```text
@repoferry/internal-build

@repoferry/internal-config

@repoferry/internal-devtools
```
...
```text
@repoferry/provider-gitlab

@repoferry/provider-bitbucket

@repoferry/provider-azure

@repoferry/provider-gitea

@repoferry/provider-local

@repoferry/provider-zip

@repoferry/cache-memory

@repoferry/cache-redis
```
...
# 9. Core Package (@repoferry/core)

The Core package is the architectural heart of RepoFerry.
...
Each provider package adapts one hosting platform to RepoFerry's provider contracts.
...
Providers translate external systems into RepoFerry concepts.
...
# 11. Shared Package (@repoferry/shared)
...
# 12. Testing Package (@repoferry/testing)
...
RepoFerry is designed for ecosystem growth.
...
```ts
import { Provider } from "@repoferry/core";
```
...
RepoFerry targets modern JavaScript ecosystems.
...
This ADR defines the package-level architecture of RepoFerry.
...
This ADR establishes the package architecture that supports the long-term evolution of RepoFerry.
...
Together with ADR-001 and ADR-002, this document provides the structural foundation upon which the remainder of RepoFerry is implemented.
