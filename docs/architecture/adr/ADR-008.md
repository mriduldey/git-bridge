...
This ADR defines how failures are represented throughout GitBridge.
...
GitBridge adopts an **Exception-based public API** with:
...
Applications interact exclusively with GitBridge errors.
...
Errors are part of GitBridge's public contract.
...
> "This is likely a GitBridge bug."
...
```text
GITBRIDGE_VALIDATION

GITBRIDGE_AUTHENTICATION

GITBRIDGE_PERMISSION_DENIED

GITBRIDGE_TIMEOUT

GITBRIDGE_CANCELLED

GITBRIDGE_RATE_LIMITED

GITBRIDGE_NOT_FOUND

GITBRIDGE_CONFIGURATION

GITBRIDGE_PROVIDER

GITBRIDGE_UNEXPECTED
```
...
1. Every public error derives from GitBridgeError.
...
GitBridge distinguishes timeout from cancellation.
...
GitBridge preserves original causes.
...
Applications may inspect causes for diagnostics but should depend only on GitBridge contracts.
...
GitBridge remains logging-framework neutral.
...
Core->>Application: GitBridgeError
```
...
This ADR defines the error architecture of GitBridge.
...
ADR-008 establishes the error model of GitBridge.

It defines:

- immutable public errors,
- provider-neutral exception hierarchy,
- stable `GITBRIDGE_*` error codes,
...
> **Failures are translated exactly once per architectural boundary, exposed through stable GitBridge contracts, and enriched with deterministic diagnostics without leaking implementation details.**

Public error types, stable error codes, and serialization formats are part of GitBridge's Semantic Versioning contract and evolve according to the compatibility guarantees established throughout the architecture.