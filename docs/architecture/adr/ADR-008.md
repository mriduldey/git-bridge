...
This ADR defines how failures are represented throughout SourceAxis.
...
SourceAxis adopts an **Exception-based public API** with:
...
Applications interact exclusively with SourceAxis errors.
...
Errors are part of SourceAxis's public contract.
...
> "This is likely a SourceAxis bug."
...
```text
SOURCEAXIS_VALIDATION

SOURCEAXIS_AUTHENTICATION

SOURCEAXIS_PERMISSION_DENIED

SOURCEAXIS_TIMEOUT

SOURCEAXIS_CANCELLED

SOURCEAXIS_RATE_LIMITED

SOURCEAXIS_NOT_FOUND

SOURCEAXIS_CONFIGURATION

SOURCEAXIS_PROVIDER

SOURCEAXIS_UNEXPECTED
```
...
1. Every public error derives from SourceAxisError.
...
SourceAxis distinguishes timeout from cancellation.
...
SourceAxis preserves original causes.
...
Applications may inspect causes for diagnostics but should depend only on SourceAxis contracts.
...
SourceAxis remains logging-framework neutral.
...
Core->>Application: SourceAxisError
```
...
This ADR defines the error architecture of SourceAxis.
...
ADR-008 establishes the error model of SourceAxis.

It defines:

- immutable public errors,
- provider-neutral exception hierarchy,
- stable `SOURCEAXIS_*` error codes,
...
> **Failures are translated exactly once per architectural boundary, exposed through stable SourceAxis contracts, and enriched with deterministic diagnostics without leaking implementation details.**

Public error types, stable error codes, and serialization formats are part of SourceAxis's Semantic Versioning contract and evolve according to the compatibility guarantees established throughout the architecture.