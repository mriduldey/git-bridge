...
GitBridge is designed to support multiple Git repository providers without exposing provider-specific implementation details to applications.
...
GitBridge adopts a **Ports & Adapters** provider architecture.
...
A provider translates provider-specific behavior into GitBridge domain concepts.

Providers are adapters.

They are not part of the public programming model.

Applications interact with GitBridge abstractions rather than providers directly.
...
GitBridge separates abstractions from implementations.
...
Capabilities implement GitBridge contracts while internally using provider-specific APIs.
...
Providers translate SDK failures into GitBridge provider failures.
...
GitBridge exposes a provider-neutral abstraction.
...
This ADR defines how providers integrate with GitBridge.
...
ADR-005 establishes the provider architecture of GitBridge.
...
This separation preserves provider neutrality while allowing GitBridge to support an arbitrary number of official and community-developed providers without requiring changes to Core.