...
SourceAxis is designed to support multiple Git repository providers without exposing provider-specific implementation details to applications.
...
SourceAxis adopts a **Ports & Adapters** provider architecture.
...
A provider translates provider-specific behavior into SourceAxis domain concepts.

Providers are adapters.

They are not part of the public programming model.

Applications interact with SourceAxis abstractions rather than providers directly.
...
SourceAxis separates abstractions from implementations.
...
Capabilities implement SourceAxis contracts while internally using provider-specific APIs.
...
Providers translate SDK failures into SourceAxis provider failures.
...
SourceAxis exposes a provider-neutral abstraction.
...
This ADR defines how providers integrate with SourceAxis.
...
ADR-005 establishes the provider architecture of SourceAxis.
...
This separation preserves provider neutrality while allowing SourceAxis to support an arbitrary number of official and community-developed providers without requiring changes to Core.