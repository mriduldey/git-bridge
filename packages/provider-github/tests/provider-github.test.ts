...
import { SourceAxisClient, RepositoryFactory } from "@sourceaxis/core";
import {
  AuthenticationError,
  AuthorizationError,
  ConflictError,
...
describe("GitHub provider foundation", () => {
  ...
  it("creates sessions with injected authentication, transport, cache, and diagnostics", async () => {
    ...
    const client = new SourceAxisClient({ authentication, providers: [provider] });
  ...
  it("expresses provider requests through SourceAxis transport", async () => {
  ...
  it("maps GitHub provider failures to approved SourceAxis errors", () => {
...
describe("public exports", () => {
  it("exports provider factory, config helper, client helper, classes, and public types", async () => {
    ...
    expect(createGitHubClient()).toBeInstanceOf(SourceAxisClient);
