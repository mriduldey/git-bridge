...
export class SourceAxisError extends Error {
  ...
  public constructor(message: string, options: SourceAxisErrorOptions = {}) {
   ...
   */
  public toJSON(): SerializedSourceAxisError {
    const serialized: {
      name: string;
      message: string;
      code: ErrorCode;
   ...
   */
  public serialize(): SerializedSourceAxisError {
    return this.toJSON();
  }
}
...
function getDefinition(name: string): ErrorDefinition {
  return definitions[name as keyof typeof definitions] ?? definitions.SourceAxisError;
}
...
function serializeCause(cause: unknown): SerializedSourceAxisError | SerializedCause | undefined {
